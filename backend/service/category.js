...
module.exports = {
    getList: async (req, res) => {
        ...
        try{
            ...
            response.list = rows;
            response.count = (await conn.query(`select count(1) as rowCount from  ${req.decodedJwt.db}.category c
            where isdeleted = 0 and companyID = ${req.query.selectedCompany} ${filterClause}`))[0].rowCount
            res.send(response);
        }
        catch(err){
