...
module.exports = {
  login: async (req, res, next) => {
    ...
    try {
      ...
      var [folders, fields] = await conn.query(
        "SELECT dbName,tenantID FROM master.tenantfolders WHERE tenantID = (SELECT tenantID FROM master.tenant WHERE tenantName = ?)",
        [req.body.tenantName]
      );
      if (!folders?.length) {
        res.status(401);
      ...
      var [rows, fields] = await conn.query(
        `SELECT * FROM ${db}.user WHERE username=? AND isActive=1 AND isDeleted=0`,
        [req.body.username]
      );
      if (!rows.length) {
        res.status(401);
  ...
  authorize: {
    ...
    search: {
      generic: async (req, res, next) => {
        if (
          !req.searchPreAuthorization ||
          !req.searchPreAuthorization.some(
            (x) => x.companyID.toString() === req.query.selectedCompany
          )
        ) {
          res.sendStatus(403);
          return;
      ...
      specific: async (req, res, next) => {
        ...
        if (!requestedPictureId) requestedPictureId = req.body.pictureID;
        let conn;
        try {
          conn = await pool.getConnection();
          var [rows, fields] = await conn.query(`
          ...
          if (
            !req.searchPreAuthorization.some(
              (x) =>
                x.companyID === rows[0].companyID &&
                x.categoryID === rows[0].categoryID
            )
          ) {
            res.sendStatus(403);
            return;
  ...
  preAuthorize: {
    search: async (req, res, next) => {
      ...
      try {
        conn = await pool.getConnection();
        var [rows, fields] =
          await conn.query(`select t.companyID, t.categoryID, c.category from ${req.decodedJwt.db}.transsecurity t
  ...
  checkLastActivity: async (req, res, next) => {
    ...
    try {
      conn = await pool.getConnection();
      var [rows, fields] = await conn.query(
        `SELECT lastactivity FROM ${req.decodedJwt.db}.user WHERE userID = ${req.decodedJwt.userId}`
      );
      if (!rows.length) {
        res.sendStatus(403);
        return;
      ...
      if (rows[0].lastactivity) {
        ...
        var minsDifference =
          (new Date(now) - rows[0].lastactivity) / (1000 * 60);
